...
class TestAtrVolScaling:
    """quality_mult should be divided by atr_ratio when ATR_VOL_SCALING=True."""

    def _df(self, n=300):
        closes = [100.0 + i * 0.1 for i in range(n)]
        return make_df(closes, highs=[c + 1 for c in closes], lows=[c - 1 for c in closes])

    def _get_mult(self, atr_ratio, atr_vol_scaling=True):
        df = self._df()
        current = float(df["close"].iloc[-1])
        levels = [
            {"price": round(current * 0.999, 2), "type": "SUPPORT", "touches": 5,
        ...
        an.ATR_VOL_SCALING = atr_vol_scaling
        sigs = an.find_entry_signals(
            df, levels, current_price=current,
            htf_structure="BULLISH", adx_val=30.0, ema_period=50,
            htf_rsi=60.0, htf_below_ema=0, atr_ratio=atr_ratio,
        )
        long_sigs = [s for s in sigs if s["direction"] == "LONG"]
        return long_sigs[0]["quality_mult"] if long_sigs else None

    def setup_method(self):
        self._orig_scaling   = an.ATR_VOL_SCALING
        self._orig_floor     = an.ATR_VOL_SCALE_FLOOR
        ...
        an.LONG_MIN_CONFLUENCE = 0
        an.ADAPTIVE_SL       = False
        an.ATR_VOL_SCALE_FLOOR = 0.4

    def teardown_method(self):
        an.ATR_VOL_SCALING   = self._orig_scaling
        an.ATR_VOL_SCALE_FLOOR = self._orig_floor
        an.QUALITY_SIZING    = self._orig_qual
        an.REQUIRE_RSI_DIVERGENCE = self._orig_rsi
        an.LONG_MIN_CONFLUENCE = self._orig_long_min
        an.ADAPTIVE_SL       = self._orig_adaptive

    def test_high_atr_ratio_reduces_mult(self):
        mult_normal = self._get_mult(atr_ratio=1.0)
        mult_high   = self._get_mult(atr_ratio=2.0)
        assert mult_normal is not None
        assert mult_high   is not None
        assert mult_high < mult_normal

    def test_floor_prevents_zero_size(self):
        mult_extreme = self._get_mult(atr_ratio=10.0)
        assert mult_extreme is not None
        assert mult_extreme >= an.ATR_VOL_SCALE_FLOOR

    def test_normal_atr_ratio_not_reduced(self):
        mult_a = self._get_mult(atr_ratio=1.0)
        mult_b = self._get_mult(atr_ratio=0.9)  # below 1.0 → no scaling
        assert mult_a is not None and mult_b is not None
        assert mult_a == mult_b  # atr_ratio ≤ 1.0 → no change

    def test_scaling_disabled_no_effect(self):
        mult_normal = self._get_mult(atr_ratio=1.0, atr_vol_scaling=False)
        mult_high   = self._get_mult(atr_ratio=2.0, atr_vol_scaling=False)
        assert mult_normal is not None and mult_high is not None
        assert mult_normal == mult_high
